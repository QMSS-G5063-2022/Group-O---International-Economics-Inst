...
map<-function(which){
  if(which=="China"){
    j<-worldmap@data %>%
      full_join(inv07, by=c(NAME="name"))
    worldmap@data<-j

    pal<-colorNumeric(palette="Reds", domain=worldmap@data$inv_tot)

    leaflet(data=worldmap) %>%
      addPolygons(weight=.5, color=~pal(inv_tot), fillOpacity = .75)
  }

  else if(which=="IMF"){
    j<-worldmap@data %>%
      full_join(imf07, by=c(NAME="name"))
    worldmap@data<-j

    pal<-colorNumeric(palette="Blues", domain=worldmap@data$IMF_cred)

    leaflet(data=worldmap) %>%
      addPolygons(weight=.5, color=~pal(IMF_cred), fillOpacity = .75)
  }}
```
